...
{
    public delegate void TableMoveHandler(int oldTableNo,int newTableN0);
    public partial class OrderForm : Form
    {
        ...
        {
            Text = $"Order (Table {_order.TableNo}-{_order.StarTime?.ToLongTimeString()})";
            lblTableNo.Text = _order.TableNo.ToString("00");
            lblTotalPrice.Text = _order.TotalPriceTRY;
            LoadEmptyTableNo();
        }

        private void LoadEmptyTableNo()
        {
            cboTableNo.DataSource = Enumerable.Range(1,_db.TableCount).
                Where(x=>!_db.ActiveOrders.Any(y=>y.TableNo==x)).ToList();
        ...
        {
            CompleteOrder("Are you sure that you want to receive the payment?",_order.TotalPrice(),OrderState.paid);
        }
        ...
        private void CompleteOrder(string message, decimal paidAmount, OrderState newState)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                DialogResult = DialogResult.OK;//Closing the form with OK 
            }
        }
